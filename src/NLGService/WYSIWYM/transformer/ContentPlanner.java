...
import NLGService.WYSIWYM.model.UndeterminedDTNode;
...
public class ContentPlanner 
{
	...
	protected int aggregationThreshold = 4;
    ...
    public ContentPlanner(OntologyReader r, SemanticGraph g, int aggregationThreshold) 
    {
    	graph = g;
    	reader = r;
    ...
    protected DependencyTreeTransformer lexicaliseEdge(SGNode topic, SGEdge edge, boolean relativeClause)
    {
   	   	...
   	   	if (object instanceof SGBooleanNode)
		{
   		...
   		if ((edgeList.size() > aggregationThreshold) && (edge.showInList() <= SGEdge.HIDDEN_LIST) && (!reader.isDatatypeProperty(edge.getLabel())))
  		{	//if there are more than 4 edges that have the same specification (and hence will be aggregated), and the user has not requested to see them all,
   			...
   			summation = true;
   			//TODO: Aggregation: remove edgeList.clear(); to include the aggregated edges in the description. 
   			edgeList.clear();
   		}
    ...
    private DependencyTreeTransformer getTree(SGEdge edge, List<String> propsCovered, SGNode topic, boolean relativeClause)
    {
		...
		OntProperty p = reader.getProperty(edgeLabel);
    	...
    	propsCovered.add(edgeLabel);   
	    DependencyTreeTransformer result = null;
	    OntProperty specProp = p;	//Property that has the specification attached
		
		try
		{
			if (edgeLabel.equals(QueryEdge.ANYTHING)){	//'anything' is obviously not a property in the ontology!
				return lexicon.map(edgeLabel);
			} else if (p == null){	//if this is not an ontology property, return null
				System.out.println("The property " + edgeLabel + " wasn't found in the OntologyReader");
	    		return null;
			} else {
		...
		if (result == null)
		{
			System.out.println("Property " + edgeLabel +	" is not in the lexicon");
			return null;
		}
		
		if (!edgeLabel.equals(specProp.getLocalName()))
			propsCovered.add(specProp.getLocalName());
		for (OntProperty subProp : reader.getSubProperties(specProp))
		{
			if (subProp.getLocalName().equals(edgeLabel))
				continue;
