package NLGService.WYSIWYM.model;
...
import NLGService.WYSIWYM.libraries.LinguisticTerms;
import NLGService.WYSIWYM.ontology.OntologyReader;
import NLGService.WYSIWYM.ontology.OntologyWriter;
import NLGService.WYSIWYM.transformer.DependencyTreeTransformer;
...
//		type = sg.getLabel();
		...
		for (Iterator it = sg.getOutgoingEdges(); it.hasNext(); ){
			SGEdge outgoingEdge = (SGEdge) it.next();
			String outgoingEdgeLabel = outgoingEdge.getLabel();
			String targetLabel = outgoingEdge.getTarget().getLabel();

			if (/*label.equalsIgnoreCase("Name") || label.equalsIgnoreCase("Title") || */ outgoingEdgeLabel.equalsIgnoreCase(OntologyWriter.LABEL_URI)){
				properName = targetLabel;
				/**
				 * TODO: Need to read class spec file for formating label
				 * 		OR simply do the whole label determination (class name + property values) in AutomaticGenerator
				 * 				--> properName = targetLabel;
				 * 				and remove everything else in the if
				 * 		if no outgoing edge label, then default behavior (e.g. a person)

				
				// Format Labels (e.g. remove email address for person label)
//				if ("person".equalsIgnoreCase(type)) {
//					targetLabel = targetLabel.substring(0, targetLabel.indexOf(" : "));
//				}
				
				// Format types:
				if ("Our Spaces Account".equalsIgnoreCase(type)) {
					type = "ourSpaces account";
				} else if ("feature".equalsIgnoreCase(type)) {
					
					if (targetLabel.toLowerCase().contains("building")) {
						type = "";
					} else {
						type = "location";
					}
				...
				// Format final label (e.g. for a person, no need to say "the person Thomas Bouttaz" --> just "Thomas Bouttaz")
				if ("person".equalsIgnoreCase(type)) {
					properName = targetLabel;
				} else {
					properName = "the " + type + " " + targetLabel;
				}
				 */
			}
			
			// If the subject is a person, must determine gender
			if (type.contains("person")) {
				// If property is foaf:gender, check if it's male or female
				//TODO gender: what do we store in foaf:gender? male/female? try with LinguisticTerms.FEMALE?
				if (outgoingEdgeLabel.equalsIgnoreCase("gender") ){
					if (targetLabel.equalsIgnoreCase(LinguisticTerms.FEMALE)) {
						setPronoun("she");
					} else if (targetLabel.equalsIgnoreCase(LinguisticTerms.MALE)) {
						setPronoun("he");
					}
				}
				//TODO: When foaf:gender used in ontology, remove this last setPronoun("he");
//				setPronoun("he");
			}
		}
		if (pronoun == null){
			if (type.contains("person")) {
				setPronoun("he");		//default pronoun for a person is male
			} else {
