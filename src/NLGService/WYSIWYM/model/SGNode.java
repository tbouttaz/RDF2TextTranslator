...
import NLGService.WYSIWYM.util.BadAnchorException;
...
public class SGNode extends Node implements Comparable
{
	...
	/** Label of this resource, as defined in the class specification (e.g. Person.xml)*/
	...
	public static SGNode copyNode(SGNode node, Map<String,SGNode> existing, SemanticGraphTransformer sg) throws NameAlreadyBoundException
	{
		...
		result.setSpecLabel(node.getSpecLabel());
		...
		//TODO: also copy Incoming Edges (but they're not realise the right way around)
...
//		}
	 ...
	 * 	If a class specification file is defined for this resource (e.g. Person.xml), use the corresponding label
	 * 
	 *	@param reader Ontology
	 *	@return String the label defined in the corresponding class specification, or just the class name
	 */
	public String getHeaderLabel(OntologyReader reader)
	{
		
		if (specLabel != null && !"".equals(specLabel)) {
			return specLabel;
			...
			List<Edge> nlEdges = getNLEdges(reader);
			if (nlEdges.size() > 0)	//append name/title
			{
				if (isQuote())
					result.append(" \"");
				else
				...
				// append the value of the "Label" edge:
				result.append(nlEdges.get(0).getTarget().getLabel());
				if (isQuote())
					result.append("\"");
			}	
