...
//		query.append(":Title ?Title.}}");
		...
		StringBuffer query = new StringBuffer();
		query.append("PREFIX Utility: <http://xmlns.com/foaf/0.1/> PREFIX Resource: <http://www.policygrid.org/provenance-generic.owl#> SELECT ?id ?surname ?title WHERE {{?id Utility:surname ?surname.} UNION {?id Resource:title ?title.}}");
		
		NLGService.WYSIWYM.ontology.QueryResult result = sesame.queryBinding(query.toString());
 	...
 	private void start(int idx)
 	{
 		try
 		{
			...
			SemanticGraphTransformer sgt = ag.buildGraphFromSesameRepo("user", id);
